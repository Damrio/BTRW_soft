...
MailChecker monMail = new MailChecker("MAIL");
MailChecker monMail_FB = new MailChecker("FB");
...
Afficheur_data bloc_facebook = new Afficheur_data();
//----------------------------------------------------------------------
...
void draw() {
  ...
  if (indicateur_mode == 1) // Il faudrait rajouter une condition : and Change_flag == true
  {
    ...
    if (arduino_enabled) {
      ...
      dessine_carre_rouge(); // dessine un carre rouge autour de la case couramment selectionnee
  ...
  if (indicateur_mode == 2) {
    ...
    if (ModeLevel==0 && mouseButton==LEFT ) { // cas ou on veut lire un flux, un mail
      ...
      if (curr_function_type.equals("FB")) {
        // on determine index du mail a afficher et mise a jour offset lecture
        bloc_facebook.pos_defil_contenu_mail_courant = 0;
        bloc_facebook.index_mail_a_afficher = round((mouseY - bloc_facebook.pos_defil_courant) / bloc_facebook.message_height);
      }
